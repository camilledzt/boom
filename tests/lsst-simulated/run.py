...
parser.add_argument(
    "--boom-repo-dir", default=".", help="Path to the boom repo directory"
)
parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds")
parser.add_argument(
    "--keep-up", action="store_true", help="Keep services up after test"
)
parser.add_argument(
    "--data-dir",
    default=None,
    help="Path to lightcurve data directory (default: <boom-repo-dir>/data_alex)",
)
parser.add_argument(
...
DATA_DIR = (
    ...
    if args.data_dir
    else os.path.join(BOOM_REPO_ROOT, "data_alex")
)
...
config["workers"]["lsst"]["alert"]["n_workers"] = 4
config["workers"]["lsst"]["enrichment"]["n_workers"] = 6
config["workers"]["lsst"]["filter"]["n_workers"] = 2
...
lc_indices_sorted = sorted(lc_indices)
if args.limit > 0:
    lc_indices_sorted = lc_indices_sorted[: args.limit]
matched = summary[summary["i"].isin(lc_indices_sorted) & (summary["n_detected"] > 0)]
expected_alerts = int(matched["n_detected"].sum())
n_active = len(matched)
n_objects = len(lc_indices_sorted)
print(
    f"Expected alerts: {expected_alerts} (from {n_active}/{n_objects} objects with detections)"
)
